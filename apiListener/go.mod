module github.com/gidoBOSSftw5731/covid-19-scraper/apiListener

go 1.13

require (
	github.com/gidoBOSSftw5731/covid-19-scraper/apiListener/goconf v0.0.0-00010101000000-000000000000
	github.com/gidoBOSSftw5731/log v0.0.0-20190718204308-3ae037c6203f
	github.com/golang/protobuf v1.4.0-rc.4.0.20200320220005-3a3cefd0f2bd
	github.com/jinzhu/configor v1.1.1
	github.com/lib/pq v1.3.0
	github.com/mattn/gomate v0.0.0-20180907095733-047a53832fa3 // indirect
	google.golang.org/protobuf v1.20.2-0.20200320194150-9d397869d892
)

replace github.com/gidoBOSSftw5731/covid-19-scraper/apiListener/goconf => ../goconf
